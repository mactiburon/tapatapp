...
# Clases y datos iniciales (copiados de tu código)
class User:
...
# Datos de ejemplo
historialCarol = LlistaHistorial()
historialCarol.afegirHistorial(HistorialTapat(data=date(2024, 12, 17), hora=time(19, 30), estat="mimido", totalHores=8))
historialCarol.afegirHistorial(HistorialTapat(data=date(2024, 12, 18), hora=time(20, 00), estat="despertao", totalHores=2))

historialJaco = LlistaHistorial()
historialJaco.afegirHistorial(HistorialTapat(data=date(2024, 12, 17), hora=time(21, 00), estat="mimido", totalHores=10))
historialJaco.afegirHistorial(HistorialTapat(data=date(2024, 12, 18), hora=time(18, 30), estat="despertao", totalHores=3))

children = [
    Nen(id=1, child_name="Carolina", sleep_average=8, treatment_id=1, time=6, informacioMedica="Al·lèrgia a la llet de vaca", historialTapat=historialCarol),
    Nen(id=2, child_name="Jacobo", sleep_average=10, treatment_id=2, time=6, informacioMedica="No té cap al·lèrgia", historialTapat=historialJaco)
]
...
def get_child(child_name):
    ...
    if child:
        return jsonify(child.__dict__), 200
...
def get_tap(tap_id):
    ...
    if tap:
        return jsonify(tap.__dict__), 200
...
def create_tap():
    ...
    taps.append(new_tap)
    return jsonify(new_tap.__dict__), 201
...
def get_treatment(treatment_id):
    ...
    if treatment:
        return jsonify(treatment.__dict__), 200
...
def get_historial(child_name):
    child = next((child for child in children if child.child_name == child_name), None)
    if child:
        return jsonify([hist.__dict__ for hist in child.historialTapat.historial]), 200
    return jsonify({"error": "Child not found"}), 404
